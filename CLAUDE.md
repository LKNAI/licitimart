...
- **Aplicação Web & API:** Next.js 15 (TypeScript, App Router, Tailwind) — esqueleto real em `src/licitimart/web/` (build e navegação verificados em navegador real), rotas `/dossies`, `/dossies/[id]`, `/pipeline`. `/dossies` já mostra **913 contratações reais do PNCP** lado a lado com 3 exemplos ilustrativos, sempre com rótulo de origem visível (`ROTULO_ORIGEM`) — nunca misturados sem distinção. Ponte feita por `scripts/exportar_para_webapp.py` (roda o coletor real, exporta JSON, sem precisar de Supabase). Falta: Supabase real, auth, automatizar a cadência de exportação.
